...
def plot_performance(benchmark_data, delay_lengths, plot_filename):
    # Plotting performance based on extracted data
    plt.figure(figsize=(10, 6))
    colors = iter(['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2'])
    extended_x = np.linspace(0, max(delay_lengths) * 1.1, 100)

    for benchmark, data in benchmark_data.items():
        color = next(colors)
        slope, intercept, r_value, p_value, std_err = linregress(delay_lengths, data)
        ...
        plt.scatter(delay_lengths, data, color=color, edgecolor='black', s=50)
...
def plot_overhead(benchmark_overhead, delay_lengths, plot_filename):
    # Plotting performance based on extracted data
    plt.figure(figsize=(10, 6))
    colors = iter(['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2'])
    extended_x = np.linspace(0, max(delay_lengths) * 1.1, 100)
...
#             coefficients = np.polyfit(delay_lengths, data, degree)
#             polynomial = np.poly1d(coefficients)
#             # extended_predictions = polynomial(extended_x)

#             # Calculate AIC for the current fit
#             residuals = data - polynomial(delay_lengths)
...
#         plt.plot(extended_x, best_polynomial(extended_x), color=best_color, linestyle='-', linewidth=2, label=f'{benchmark}: {best_polynomial}')
#         # for benchmark, data in benchmark_overhead.items():
#         plt.scatter(delay_lengths, data, color=color, edgecolor='black', s=50)
...
#         plt.axhline(y=lower_limit_constant, color='gray', linestyle='--', label=f'Lower Limit (y = {lower_limit_constant:.2f})')
    ...
    #     spline = UnivariateSpline(delay_lengths, data, s=1)
...
def main():
    ...
    if params:
        for i, (threads, outer_repetitions, test_time, delay_length, delay_time) in enumerate(params, start=1):
            delay_lengths.append(int(delay_length))
    ...
    plot_performance(benchmark_data, delay_lengths, args.output_dir+'_performance_plot.png')
    plot_overhead(benchmark_overhead, delay_lengths, args.output_dir+'_overhead_plot.png')
