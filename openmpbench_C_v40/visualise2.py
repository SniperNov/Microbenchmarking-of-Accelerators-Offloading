...
def remove_outliers_iqr(data, delays):
...
def plot_performance(data1, data2, tag, delays, job, MIDA, graph, machine):
    ...
    # Plot data1
    for measured, data in data1.items():
        filtered_data, filtered_delays = remove_outliers_iqr(data, delays)

        color = next(colors)
        slope, intercept, r_value, p_value, std_err = linregress(filtered_delays, filtered_data)
        analyse_dist(measured, directory, slope, intercept, std_err, filtered_delays)
        extended_predictions = intercept + slope * extended_x

        plt.plot(extended_x, extended_predictions, color=color, linestyle='-', linewidth=2, label=f'{measured} (y = {slope:.4f}x + {intercept:.4f})')
        plt.scatter(filtered_delays, filtered_data, color=color, edgecolor='black', s=50)
        all_y_values.extend(data)

    # Plot data2
    for subtracted, data in data2.items():
        filtered_data, filtered_delays = remove_outliers_iqr(data, delays)
        color = next(colors)
        slope, intercept, r_value, p_value, std_err = linregress(filtered_delays, filtered_data)
        analyse_dist(subtracted, directory, slope, intercept, std_err, filtered_delays)
        extended_predictions = intercept + slope * extended_x

        plt.plot(extended_x, extended_predictions, color=color, linestyle='-', linewidth=2, label=f'{subtracted} (y = {slope:.4f}x + {intercept:.4f})')
        plt.scatter(filtered_delays, filtered_data, color=color, edgecolor='black', s=50)
        all_y_values.extend(data)
    ...
    if graph:
        ...
        # Adjust the y-axis limits to include negative points if present
        # y_min = min(all_y_values) if all_y_values else 0
        # y_max = max(all_y_values) * 1.1 if all_y_values else 0
...
def main():
    ...
    reference_dict = {"Function 2 execution time measured": reference_data}
    all_dict = {"Function 1 and 2 execution time measured": all_data}
    observed_dict = {"Function 1 execution time measured": observed_data}
    expected_dict = {"Function 1 execution time subtracted": expected_data}
    ...
    plot_performance(reference_dict, all_dict, 'refall', delays, job, method + '_' + IDA, True, machine)
    plot_performance(observed_dict, expected_dict, 'func1', delays, job, method + '_' + IDA, True, machine)
